...
export const ChildNodes = (props) => {
	...
	return (
		...
		// 		child_xy.nodePositions.map((pos, i) => {
					...
					return (
						<div className={styles.child_nodes}>
						{child_xy.nodePositions && child_xy.textPositions &&
							child_xy.nodePositions.map((pos, i) => {
								const isRightSide = child_xy.angles[i] > -Math.PI/2 && child_xy.angles[i] < Math.PI/2;
								const style = {
									left: `${pos[0]}px`,
									top: `${pos[1]}px`,
									position: 'absolute',
								...
								return (
									<div className={styles.child} key={i} style={style}>
										<p className={styles.title} style={{ width: 'max-content', border: 'solid 2px red', textAlign: 'center', position: 'absolute' }}>
											{props.nodes[i].name}
										</p>
									</div>
								)
				...
				Removed setParams and directly created a style object within the map function.
				Used a ternary operator to set justifyContent based on the angle, to handle the positioning of text relative to its node for left and right sides of the circle.
