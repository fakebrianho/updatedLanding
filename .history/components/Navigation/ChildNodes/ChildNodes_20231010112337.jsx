...
export const ChildNodes = (props) => {
	...
	const ChildParentNode = ({ style, children, key }) => {
		return (
			<div className={styles.parent} style={style}>
				<ChildNode className={styles.child}>{children}</ChildNode>
			</div>
		)
	}
			...
			{child_xy.nodePositions &&
				...
				child_xy.nodePositions.map((pos, i) => {
					const isRightSide =
						...
						child_xy.angles[i] < Math.PI / 2
					const isParent = props.nodes[i].num_grandchild_nodes > 0
					const style = {
						left: `${isParent ? pos[2] : pos[2]}px`,
						top: `${isParent ? pos[3] : pos[3]}px`,
						position: 'absolute',
						display: 'flex',
						justifyContent:
