...
export default function About() {
	...
	useEffect(() => {
		...
		if (textRef.current) {
			...
			// Animate each word with 3D rotation
			shuffledWords.forEach((word) => {
				tl.to(
					word,
					{
						duration: 0.6,
						delay: Math.random() * 0.5, // Random delay for each word
						opacity: 1,
						scale: 1,
						y: 0,
						rotationX: 360 * Math.random(), // Random rotation along X axis
						rotationY: 360 * Math.random(), // Random rotation along Y axis
						ease: 'back.out(1.7)',
					},
					0
				)
