...
export async function getMarginalia(file_name) {
    try {
        ...
        const col = client.db(process.env.DB_NAME).collection("first-principles");

        const post = await col.findOne({ file_name: file_name });
        if (!post) {
            console.log("Fetching—was not able to locate post with file name: ", file_name);
            return null;
        }
...
export async function addMarginalia(file_name, marg) {
    try {
        ...
        console.error("Adding new marginalia—was not able to locate post with file name: ", file_name);
        return null;
    } catch (e) {
        console.error(e);
    }
